...
import {ILM_ManualExternalPriceSetter_v1} from "src/modules/fundingManager/oracle/interfaces/ILM_ManualExternalPriceSetter_v1.sol";
...
contract LM_ManualExternalPriceSetter_v1_Test is Test {
    ...
    */
    function testInit_GivenValidParams() public {
        ...
        LM_ManualExternalPriceSetter_v1_Exposed newPriceSetter = new LM_ManualExternalPriceSetter_v1_Exposed();
        ...
        uint256 testPrice = 1000000; // 1 USDC
        uint256 normalizedPrice = newPriceSetter.exposed_normalizePrice(testPrice, 6);
        assertEq(normalizedPrice, 1000000000000000000); // 1e18 (normalized to 18 decimals)
    ...
    */
    function testNormalizePrice_GivenValidPrice(uint256 price) public {
        vm.assume(price < type(uint256).max / 10**18); // Prevent overflow
        
        uint8 decimals = 6; // USDC-like decimals
        uint256 normalizedPrice = priceSetter.exposed_normalizePrice(price, decimals);
        uint256 denormalizedPrice = priceSetter.exposed_denormalizePrice(normalizedPrice, decimals);
        
        assertEq(price, denormalizedPrice, "Price should remain the same after normalization and denormalization");
    }
    ...
    */
    function testSetIssuancePrice_GivenValidPrice(uint256 price) public {
        vm.assume(price > 0);
        vm.assume(price < type(uint256).max / 10**18); // Prevent overflow
    ...
    */
    function testSetRedemptionPrice_GivenValidPrice(uint256 price) public {
        vm.assume(price > 0);
        vm.assume(price < type(uint256).max / 10**18); // Prevent overflow
    ...
    */
    function testSetPrice_revertGivenZeroPrice() public {
        ...
        vm.expectRevert(ILM_ManualExternalPriceSetter_v1.Module__LM_ExternalPriceSetter__InvalidPrice.selector);
    ...
    */
    function testSetPrice_revertGivenUnauthorizedCaller(address unauthorized) public {
        ...
        bytes memory revertData = abi.encodeWithSignature(
            "Module__MissingRole(bytes32,address)",
            role,
            unauthorized
        );
        
        vm.expectRevert(revertData);
        priceSetter.setIssuancePrice(1000000);
        
        vm.expectRevert(revertData);
        priceSetter.setRedemptionPrice(1000000);
